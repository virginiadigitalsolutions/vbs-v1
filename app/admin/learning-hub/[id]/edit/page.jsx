...
export default function EditBlogPost({ params }) {
    ...
    const addBlock = (type) => {
        ...
        setBlocks([...blocks, { id: blockId, type, data }])
    }

    const updateBlock = (blockId, newData) => {
        setBlocks(blocks.map(b => b.id === blockId ? { ...b, data: { ...b.data, ...newData } } : b))
    }
    ...
    useEffect(() => {
        fetch(`/api/admin/learning-hub/${id}`)
            .then(res => res.json())
            .then(post => {
                if (post?.id) {
                    setFormData({
                        ...
                        featuredImgAlt: post.featuredImgAlt || '',
                        publishedAt: toDatetimeLocalValue(post.publishedAt || post.createdAt),
                    ...
                    try {
                        ...
                        if (Array.isArray(parsed)) {
                            ...
                            // Legacy HTML content handled properly
                            setBlocks([{ id: 'legacy', type: 'text', data: { html: post.content } }])
                        }
                    } catch (e) {
        ...
        fetch('/api/admin/learning-hub/categories').then(res => res.json()).then(setCategories).catch(console.error)
        fetch('/api/admin/learning-hub/authors').then(res => res.json()).then(setAuthors).catch(console.error)
    }, [id])
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await fetch(`/api/admin/learning-hub/${id}`, {
                method: 'PATCH',
    ...
    // Removed legacy handleDrop

    const handleDelete = async () => {
        const confirmed = await showConfirm({
            title: 'Delete this post?',
            text: 'This cannot be undone. The article will be permanently removed.',
            confirmText: 'Yes, delete it',
        })
        if (!confirmed) return

        try {
            await fetch(`/api/admin/learning-hub/${id}`, { method: 'DELETE' })
    ...
    if (loading) return <div className="min-h-screen flex justify-center pt-20 font-bold text-gray-400">Loading editor...</div>
        ...
        <div className="min-h-screen bg-[#fafafa]">
            ...
            <div className="bg-white/80 backdrop-blur-xl border-b border-gray-100 px-6 py-4 flex items-center justify-between sticky top-0 z-40 shadow-sm shadow-gray-100/50">
                <div className="flex items-center gap-4">
                    ...
                    <span className="font-extrabold text-gray-900 tracking-tight">Edit Article</span>
                    {generating && <span className="text-xs font-bold text-violet-500 bg-violet-50 px-2 py-1 rounded animate-pulse">🤖 Generating...</span>}
                </div>
