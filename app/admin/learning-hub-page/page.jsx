...
import { HiOutlineEye, HiOutlinePencil, HiOutlineChevronUp, HiOutlineChevronDown, HiOutlineSparkles, HiOutlineTemplate, HiOutlineGlobe } from 'react-icons/hi'
...
const layoutLabels = {
    learning_hub_hero: 'Learning Hub Hero',
    learning_hub_feed: 'Learning Hub Feed',
}
    ...
    useEffect(() => {
        async function load() {
            try {
                ...
                const page = pages.find(p => p.slug === 'learning-hub')
        ...
        try {
            const pageRes = await fetch('/api/admin/pages', {
                ...
                body: JSON.stringify({
                    title: 'Learning Hub',
                    slug: 'learning-hub',
                    metaDesc: 'Insights, tutorials, and practical learning resources from the VBS team.',
                    isPublished: true,
                }),
            })
            ...
            if (!pageRes.ok) {
                throw new Error(page.error || 'Failed to create Learning Hub page')
            }
            ...
            for (const section of defaultSections) {
                ...
                if (!secRes.ok) {
                    throw new Error(sec.error || 'Failed to create default Learning Hub sections')
                }
            ...
            showSuccess('Learning Hub page created successfully!')
        } catch (err) {
            showError(err.message || 'Failed to create Learning Hub page')
        } finally {
            setCreating(false)
        }
    }
            ...
            <div className="max-w-3xl mx-auto px-6 py-20 text-center">
                <div className="bg-white rounded-2xl border border-gray-100 p-12 shadow-sm">
                    <div className="w-16 h-16 mx-auto mb-6 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-center">
                        <HiOutlineTemplate className="text-2xl text-gray-400" />
                    </div>
                    <h2 className="text-xl font-bold text-gray-900 mb-2">Page Not Found</h2>
                    <p className="text-gray-500 text-sm mb-6">The Learning Hub page record does not exist yet. Create it once and this CMS screen will be ready to use.</p>
                    <button
                        onClick={handleCreatePage}
                        disabled={creating}
                        className="inline-flex items-center rounded-xl bg-primary-600 px-5 py-3 text-sm font-bold text-white shadow-sm transition-colors hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                        {creating ? 'Creating Learning Hub Page...' : 'Create Learning Hub Page'}
                    </button>
                </div>
            </div>
        )
            ...
            <div className="max-w-4xl mx-auto px-6 py-10">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="flex items-start gap-4">
                            <div className="w-10 h-10 rounded-xl bg-primary-50 border border-primary-100 flex items-center justify-center shrink-0">
                                <HiOutlineTemplate className="text-lg text-primary-600" />
                            </div>
                            <div>
                                <h1 className="text-xl font-bold text-gray-900 tracking-tight">Learning Hub Page</h1>
                                <p className="text-gray-500 text-sm mt-0.5">
                                    Manage layout sections for the Learning Hub landing page.
                                </p>
                ...
                <div className="space-y-3">
                    ...
                    {sorted.map((section, idx) => {
                        const meta = sectionMeta[section.type] || sectionMeta.cards
